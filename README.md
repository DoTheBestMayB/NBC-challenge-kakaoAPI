...
- [x] 기본 요구사항
- [x] 디바이스에 지난 검색어 저장(삭제 가능하도록)
- [ ] 이미지 리스트에 반복적으로 쓰이는 아이템은 커스텀뷰로 제작
- [ ] 사진/영상 클릭시 다음 액티비티에서 확대/재생 기능 추가
- [ ] 테스트 코드 작성
...
LocalDate : 날짜를 표기하는 데 사용하는 java.time 패키지 클래스 </br>
LocalTime : 시간을 표기하는 데 사용하는 java.time 패키지 클래스 </br>
LocalDateTime : LocalDate + LocalTime

EpochTime : 1970-01-01 00:00:00 UTC </br>
...
@Body : Request Body에 요청 파라미터를 넣는다. </br>
...
GET, DELETE method는 Body를 사용하지 않는다. 반면 POST, PUT method는 Body가 있어야만 한다. </br>
...
GET에 Body를 사용하려한 이유는, 사용하지 못한다는 것을 몰랐고, query parameter를 전달하는 것보다 Body를 통해 전달하는 것이 더 안전하다고 생각했기 때문이다. </br>
...
Kakao API 명세에 문저 작성시간 포맷이 `[YYYY]-[MM]-[DD]T[hh]:[mm]:[ss].000+[tz]` 라고 적혀있다. 괄호 `[`, `]`는 선택적으로 제공될 수도 있다는 의미이다. </br>
예를 들어, `2019-01-09T15:55` 입력이 주어질 때, 아래의 format 으로는 parsing 할 수 없다.

`yyyy-MM-dd'T'HH:mm:ss.SSSXXX`

초 단위 아래가 제공되지 않았기 때문으로, optional의 의미인 괄호를 추가해줘야 한다. </br>
이때, symbol 앞의 `:`, `.` 등도 포함해야 함에 주의하자.
